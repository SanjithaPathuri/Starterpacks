	...
	$.ajax({
		...
		success: function(data)
		{
			...
			$.each(data.Roles,function(i){
				...
				result+="<tr><td align='center'><input type='checkbox' name='vehicle' value='"+row.role_id+"'></td><td>"+row.role_name+"</td><td>"+row.created_date+"</td><td class='text-center'>"+status+"</td><td class='text-center'><button data-toggle='modal' data-target='#testuser' OnClick='editfun("+row.role_id+");' class='btn btn-xs btn-primary mar-b5 bt-r mar_b5' type='button' title='Edit'><i class='fa fa-pencil'></i></button> <button data-toggle='modal' data-target='' OnClick='del("+row.role_id+");' class='btn btn-xs btn-danger mar-b5 bt-r mar_b5' type='button' title='Delete' ><i class='fa fa-trash-o'></i></button></td></td></tr>";
			});
...
function del(id)
{
	alertify.confirm("Do you eally want to delete this Role ?", function (e) {
		if (e) 
		{
			var ids = [];
			ids.push(id);
