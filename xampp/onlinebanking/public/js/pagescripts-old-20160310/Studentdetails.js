		...
		$.ajax({
			...
			success: function(data)
			{
				...
				$.each(data.Students,function(i){
					...
					result+="<tr><td><input type='checkbox' name='student' value='"+item.UserId+"'></td><td>"+item.Email+"</td><td>"+item.FirstName+"&nbsp"+item.LastName+"</td><td>"+item.MobileNo+"</td><td align='center'><img style='height:20px' src='"+item.PhotoImagePath+"'/></td><td align='center'>"+status+"</td><td align='center'><a href='http://www.clicktowish.in/smartbridge/AdminStudents/editstudent/"+item.UserId+"' class='btn btn-xs btn-primary mar-b5 bt-r mar_b5'><i class='fa fa-pencil'></i></a> <button data-toggle='modal' data-target='' OnClick='del("+item.UserId+");' class='btn btn-xs btn-danger mar-b5 bt-r mar_b5' type='button' title='Delete' ><i class='fa fa-trash-o'></i></button></td></td></tr>";
				});
