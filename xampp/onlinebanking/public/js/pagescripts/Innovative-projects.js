...
function get_Innovative(page)
{
	...
	var sort = $.trim($("[name=filters]:checked").attr('did'));
	$.ajax({
		url: "",
		type: "POST",
		data:{'get':'Innovative','page':page,'Category':Category,'Branch':Branch,'Sort':sort},
		beforeSend: function()
		{
			$("#Innovativepro").hide();
			$("#pre-loading").show();
		...
		success:function(data)
		{
			...
			if(data.Projects!='')
			{
				$.each(data.Projects,function(i){
					...
					var splitted = row.ipCreatedDate;
					...
					******************************************************************/
					var label = '';
					if(row.Label!='')
						label = "<div class='bxbk-undfdbtn'><a href='javascript:void(0);'><span class='btn btn-danger btn-xs'>"+row.Label+"</span></a></div>";
					var url = base_url+"InnovativeProjects/Details/"+row.ProjectId;
					html+="<div class='col-lg-4 col-md-4 col-sm-4 col-xs-12'><div class='bxblock'><div class='bxbk-img'><img class='img-responsive' src='"+Image+"'/></div>"+label+"<div class='bxbk-name'>"+row.ProjectTitle+New+"</div><div class='bxbk-star-vwdtls'><div class='star-wrps'>"+Rating+"</div><a href='"+url+"' class='viewdlsbtn btn btn-info btn-sm'>View Details</a> </div> <div class='bxbk-addcart-pric'> <span class='bxbk-price'><i class='fa fa-rupee'></i> "+row.ipPrice+"</span></div></div></div>";
				});
			}
			else
