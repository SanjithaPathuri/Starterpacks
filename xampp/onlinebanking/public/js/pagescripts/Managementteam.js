	...
	$('#image').change(function(){
		...
		else
		{
			...
			img.onload = function () {
				...
				{
					err = '1';
					alertify.alert("Please upload Images higher than 500 x 500 resolution");
					$('#image').val('');
					return false;
				}
				else
...
function editfun(teamid)
{
	...
	$.ajax({
			type: "POST",
			url: base_url+"Managementteam/EditMember",
			data : "teamid="+teamid,
			success: function(data)
			{
				...
				$("#Mtimg").attr("src",base_url+obj.Mtimg);
		...
		success:function(data)
		{
			...
			if(data.Managements.length>0)
			{
				...
				if(item.Mtimg!= '')
					...
					html+='<tr><td>'+item.Name+'</td><td>'+item.Branch+'</td><td align="center">'+Mtimg+'</td><td>'+comm+'</td>'+status+'<td align="center"><button data-toggle="modal" data-target="" OnClick="editfun('+item.Id+');" class="btn btn-xs btn-primary mar-b5 bt-r mar_b5" type="button" title="Edit" ><i class="fa fa-pencil"></i></button> <button data-toggle="modal" data-target="" OnClick="deletefun('+item.Id+');" class="btn btn-xs btn-danger mar-b5 bt-r mar_b5" type="button" title="Delete" ><i class="fa fa-trash-o"></i></button></td></tr>';
				});				
			...
			{
				$("#management_table").find('tbody').html("<tr><td colspan='7'><div class='alert alert-danger mar_b0'> No Records Found..! </div></td></tr>")
				$("#pagination").hide()
			}
		}					
	})
		...
		if(err == '') 
		{
			...
			$.ajax({
					...
					success: function(data1)
					{
						...
						{
							alertify.alert("<p style='color:red'>"+datass.msg+"</p>");search_management(cpage);
						}
					}
				});
		}
...
function deletefun(teamid)
{
	alertify.confirm("Do you really want to delete this Banner ?", function (e) 
	{
		if (e) 
		{
			$.ajax({
				type: "POST",
				url: base_url+"Managementteam/DeleteMember",
				data : "teamid="+teamid,
				success: function(data)
				{
