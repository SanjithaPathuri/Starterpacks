...
$(function(){
	...
	$('#DiscountAmount').blur(function(){
		...
		if(price!='')
		{
			if(parseInt(price) < parseInt(DiscountAmount))
			{
				$('#DiscountAmount').val('');
				$('#Price').css('border','1px solid red');
				$('#DiscountAmount').css('border','1px solid red');
				alertify.alert("Discount Price cannot be greater than Actual Price..");
			}
			else
			{
				$('#Price').css('border','');
	...
	$('#Bdiagram').change(function(){
		...
		else
		{
			...
			img.onload = function () {
				...
				{
					err = '1';
					alertify.alert("Please upload Images with atleast 700 x 700 resolutions");
					$('#Bdiagram').val('');
					return false;
				}
			};
	...
	$.ajax({
		...
		success: function(data)
		{
			...
			$.each(data.Projects, function(i){
				...
				html+="<tr><td>"+row.ProjectTitle+"</td><td align='center'><img src='"+diag+"' style='height:25px;'/></td><td align='center'><a href='"+base_url+row.Resources+"' target='_blank'>Resources</a></td><td align='center'>"+status+"</td><td align='center'><button title='Edit' type='button' class='btn btn-xs btn-primary mar-b5 bt-r mar_b5' onclick='editfun("+row.ProjectId+");'><i class='fa fa-pencil'></i></button> <button title='Delete' type='button' class='btn btn-xs btn-danger mar-b5 bt-r mar_b5' onclick='delfunc("+row.ProjectId+");'><i class='fa fa-trash'></i></button></td></tr>";
			});
...
function reset()
{
	...
	$('#SubscriptionId').css('border','');  // select2
	$('#SubscriptionId').select2('val',''); //select2
