					...
					success:function(data)
					{
						...
						if(data.Clients.length>0)
						{
							$.each(data.Clients, function(i){
								...
								html+='<tr><td>'+item.Name+'</td><td align="center"><img style="height:20px" src="'+path+'"></td>'+status+'<td align="center"><button data-toggle="modal" data-target="" OnClick="editfun('+item.ClientId+');" class="btn btn-xs btn-primary mar-b5 bt-r mar_b5" type="button" title="Edit" ><i class="fa fa-pencil"></i></button> <button data-toggle="modal" data-target="" OnClick="deletefun('+item.ClientId+');" class="btn btn-xs btn-danger mar-b5 bt-r mar_b5" type="button" title="Delete" ><i class="fa fa-trash-o"></i></button></td></tr>';
							});				
						...
						{
							$("#client_table").find('tbody').html("<tr><td colspan='4'><div class='alert alert-danger mar_b0'> No Records Found..! </div></td></tr>")
							$("#pagination").hide()
						}
					}					
				})
