...
function editfun(LatestId)
{
	...
	$.ajax({
		type: "POST",
		url: base_url+"LatestNews/EditNews",
...
function deletefun(LatestId)
{
	if(confirm("Are you sure You want to delete this record?"))
	{
		$.ajax({
				type: "POST",
				url: base_url+"LatestNews/DeleteNews",
				data : "LatestId="+LatestId,
				success: function(data)
				{
...
function search_news(page)
			{
				...
				url: base_url+"LatestNews/search",
					...
					success:function(data)
					{
						...
						{
							$("#News_table").find('tbody').html("<tr><td colspan='3'><div class='alert alert-danger mar_b0'> No Records Found..! </div></td></tr>")
							$("#pagination").hide()
						}
					}					
				})
