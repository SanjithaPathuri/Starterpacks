		...
		success:function(data)
		{
			...
			if(data.Internships.length>0)
			{
				$.each(data.Internships,function(i){
					...
					var cdate = item.CreatedOn.split(' ');
					if(item.istatus == '1')
					...
					html+='<tr><td>'+item.Internship_Heading+'</td><td>'+item.CompanyName+'</td><td>'+item.Location+'</td><td align="center">'+ddate[0]+'</td><td align="center">'+item.Duration_number+'</td>'+Status+'<td align="center"><button data-toggle="modal" data-target="" OnClick="editfun('+item.Internship_Id+');" class="btn btn-xs btn-primary mar-b5 bt-r mar_b5" type="button" title="Edit" ><i class="fa fa-pencil"></i></button> <button data-toggle="modal" data-target="" OnClick="deletefun('+item.Internship_Id+');" class="btn btn-xs btn-danger mar-b5 bt-r mar_b5" type="button" title="Delete" ><i class="fa fa-trash-o"></i></button></td></tr>';
				});				
			...
			{
				$("#Internship_table").find('tbody').html("<tr><td colspan='8'><p style='margin-bottom:0px; text-align:center;'>No Records Found...!</p> </td></tr>")
				$("#pagination").hide()
			}		
		}
	});
